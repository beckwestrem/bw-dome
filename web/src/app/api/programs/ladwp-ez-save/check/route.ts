...
import { checkLadwpEzSaveEligibility } from "@/programs/ladwp_ez_save/rules";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(checkLadwpEzSaveEligibility(parseBody(body)));
  } catch (error) {
    const message = error instanceof Error ? error.message : "Server error";
    console.error("[ladwp-ez-save/check]", message);
    return NextResponse.json(
      { error: "Could not check EZ-SAVE eligibility right now." },
      { status: 500 },
    );
  }
}
